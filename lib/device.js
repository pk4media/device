...
var defaultOptions = {
    ...
    emptyUserAgentCompanyType: 'Unknown',
    unknownUserAgentDeviceType: 'phone',
    unknownUserAgentCompanyType: 'Unknown',
...
function DeviceParser(user_agent, options) {
    ...
    self.get_type = function () {
        ...
        if (ua.match(/adbeat\.com/i))
            ua = 'adbeat.com';
        if (ua.match(/evaliant/i))
            ua = 'evaliant';
        if (ua.match(/AppleWebKit\/537\.36/i) && ua.match(/Chrome\/50.0.2661.102/i) && ua.match(/Safari\/537\.36/i)) {
            ua = 'evaliant';
        }
